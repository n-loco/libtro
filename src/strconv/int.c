...
size_t tro_int2str16(int64_t num, char16_t *out, size_t outcap)
{
	if (out != NULL && outcap < 2) {
		if (outcap > 0)
			*out = '\0';
	...
	if (num < 0) {
		...
		return tro_uint2str16(unum, uout, uoutcap) + 1;
	}

	return tro_uint2str16(num, out, outcap);
}

size_t tro_uint2str16(uint64_t num, char16_t *out, size_t outcap)
{
	if (out != NULL && outcap < 2) {
		if (outcap > 0)
			*out = '\0';
